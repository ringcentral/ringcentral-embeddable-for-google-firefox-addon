...
class ClickToDialInject {
  constructor({
    ...
    this._onC2DNumberMouseEnter = this._onC2DNumberMouseEnter.bind(this);
    this._onC2DNumberMouseLeave = this._onC2DNumberMouseLeave.bind(this);
    this._initObserver();
    this._injectC2DMenu();
  }
  ...
  _handlePhoneNumberNodes(nodes) {
    nodes.forEach((node) => {
      ...
      const results = libphonenumber.findNumbers(node.data, 'US', { v2: true });
      if (results.length === 0) {
        return;
      }
  ...
  onCallClick() {
    console.log('c2d:', this._currentNumber);
    this._onCallClick(this._currentNumber);
  }

  onSmsClick() {
    console.log('c2sms:',this._currentNumber);
    this._onSmsClick(this._currentNumber);
  }
}

window.ClickToDialInject = ClickToDialInject;
